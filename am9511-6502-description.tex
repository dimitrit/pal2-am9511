...
of Kissel and Currie\footnote{Kissel, R and Currie, J: 
...
and the microprocessor are effectively coordinated, despite the lack of a 
shared clock.

The bus signal timing requirements of the AM9511 are generally easy to 
satisfy using microprocessors such as the 8080, 8085, or Z80, which
...
the end of a write operation) and the rise of the $\overbar{CS}$ signal. 
...
monostable multivibrator. This specific part is used because it can deliver
an output pulse of a fixed duration, without regard to the length of the 
trigger pulse. The combination of a 68 pF capacitor and a 2.4 K\textohm\  resistor 
...
shorter than one half of the $PHI2$ clock period (assuming a clock speed of 2
MHz or less).
...
the $\overbar{RD}$ input and the rise of the $\overbar{CS}$ input is 
given as zero --- in particular, this implies that $\overbar{RD}$ 
cannot rise \emph{after} $\overbar{CS}$.
...
for this purpose. A complication arises from the fact that the 6502 
observes the state of this input on the rising edge of the $PHI2$ 
clock --- i.e. at the instant at which the 6502 would perform the read 
operation, if $RDY$ is low the 6502 will instead wait until the next 
rising edge of the clock at which time it will again observe 
...
operation was initiated. This implies that we must arrange to pull the
RDY signal low \emph{before} the first rising edge in a machine cycle
that will read from the AM9511, anticipating that the AM9511 will
subsequently assert $\overbar{PAUSE}$.

Asserting the AM9511's $\overbar{RD}$ after the falling edge of $PHI2$ 
is complicated by the fact that the 6502 can arbitrarily change the 
state of the address lines and $R/\overbar{W}$ immediately after the 
falling edge of the $PHI2$ clock. We must be careful to avoid asserting 
$\overbar{RD}$ spuriously while the 6502's signals are settling --- 
doing so could inadvertently trigger a read operation, unexpectedly 
disrupting the state of the AM9511's stack.
