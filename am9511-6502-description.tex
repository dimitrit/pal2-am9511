...
a clock speed no greater than 3 MHz. Of course, the AM9511 could be clocked 
from the system clock (with an appropriate divider circuit if necessary).
...
to usethe time during which an operation is executing. One simple approach is 
to simply poll the BUSY flag in the flag in the AM9511's status register to 
determine when a requested operation has completed execution. 

Alternatively, the hardware could make use of the $\overbar{END}$ (or the 
$SVREQ$ signal) as an interrupt source to allow other work to be completed 
concurrently while the AM9511A is executing operations. The module described 
...
sequence of interdependent operations in main memory, placing operands and
operations on the AM9511's stack as soon as it is able to accommodate them.
...
satisfies what amounts to the most critical timing constraint of the AM9511A.
...
in Figure \ref{fig:read-timing}, $\overbar{RD}$ has essentially the same timing as 
$\overbar{CS}$ so they can be considered equivalent for our purposes here.
...
logic one, causing the first flip-flop to reset and asserting $RDY$. This allows 
the read operation to be completed at the next rising edge of $PHI2$.
